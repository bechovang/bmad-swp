...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Giữ chỗ có tiền:} khách đặt cọc \textbf{10\%} để giữ unit (reservation), quá hạn check-in thì unit tự trả về Available và cọc xử lý theo policy (v1: no-show mất cọc).
...
Facility Manager & Do hệ thống provisioning & Quản lý facility/zone/unit; đặt lịch trực; quyết định escalate severe (Maintenance + tái định vị); merge/retire unit. \\ \hline
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Đặt chỗ \& thuê (3):} \entity{reservations}, \entity{extensions}, \entity{checkout\_requests}.
    \item \textbf{Hợp đồng \& phụ lục (2):} \entity{contracts}, \entity{contract\_addendums}.
...
\subsubsection{RESERVATIONS (ĐẶT CHỖ -- THUÊ -- BK-)}
\textbf{Mô tả:} Khách chọn unit trên Browse và đặt cọc 10\% để giữ chỗ. Theo quyết định V3 (2026-09-18), bảng \entity{rentals} cũ được \textbf{gộp vào đây}: một bản ghi đi trọn vòng đời đặt chỗ $\rightarrow$ thuê $\rightarrow$ trả, vì cọc duy trì phiên (không tồn tại ``booking không có phiên thuê''). Sau check-in, \attr{CheckedIn} chính là trạng thái ``đang thuê'': bản ghi nắm access code, tiền cọc đang giữ và ngày kết thúc (dịch ngay khi gia hạn). Ngày bắt đầu trễ hơn (turnover buffer) giúp unit cũ kịp Preparing. Vòng đời đầy đủ ở state chart Reservation (Section 3).
...
\attr{EndDate} & \sqltype{DATE} & Ngày kết thúc hiện tại (dịch khi gia hạn) & NOT NULL, $>$ \attr{StartDate} \\ \hline
\attr{DepositAmount} & \sqltype{DECIMAL(18,2)} & Cọc 10\% giữ suốt phiên & NOT NULL \\ \hline
\attr{AccessCode} & \sqltype{VARCHAR(10)} & Mã mở khoá, cấp khi check-in & NULL \\ \hline
\attr{Status} & \sqltype{ENUM('PENDING\_PAYMENT','RESERVED','CHECKED\_IN','CHECKOUT\_REQUESTED','CLOSED','EXPIRED','CANCELLED')} & Trạng thái (gộp vòng đời thuê) & NOT NULL \\ \hline
\multicolumn{4}{|l|}{\attr{CANCELLED} dự phòng theo policy -- v1 không kích hoạt (PRD §6 Non-Goals)} \\ \hline
\end{xltabular}

\subsubsection{EXTENSIONS (GIA HẠN)}
\textbf{Mô tả:} Chuỗi kiểm toán mỗi lần khách kéo dài thuê: lưu ngày cũ $\rightarrow$ ngày mới và phí gia hạn. Ngày kết thúc trên \entity{reservations} được cập nhật ngay khi phí thanh toán thành công; bản ghi extension là bằng chứng để đối chiếu và sinh phụ lục hợp đồng (bảng \entity{contract\_addendums}).
...
\attr{ReservationID} & \sqltype{BIGINT} & Phiên đặt chỗ -- thuê được gia hạn & FK $\rightarrow$ RESERVATIONS \\ \hline
...
\textbf{Mô tả:} Yêu cầu trả kho của khách. Khi gửi, reservation chuyển CheckoutRequested và sinh task checkout cho staff trực zone; ngày trả mới nhất luôn thắng (``Latest new checkout'') để không đè reservation kế tiếp.
...
\attr{ReservationID} & \sqltype{BIGINT} & Phiên đặt chỗ -- thuê & FK $\rightarrow$ RESERVATIONS \\ \hline
...
\subsubsection{CONTRACTS (HỢP ĐỒNG GỐC -- CT-)}
\textbf{Mô tả:} Hệ thống \textbf{tự soạn thảo} hợp đồng gốc (auto-draft) ngay khi đặt cọc thành công -- mỗi đặt chỗ sinh \textbf{đúng một} hợp đồng (1:1 với \entity{reservations}), snapshot nội dung theo phiên bản policy đã khoá. Hợp đồng \textit{không bao giờ bị sửa}: mọi thay đổi sau này (gia hạn, đổi ngày kết thúc) đi qua \textbf{phụ lục} -- bản ghi riêng ở bảng \entity{contract\_addendums} (V3 2026-09-18, thay cho self-FK \attr{ParentContractID} cũ). Ảnh bản ký là điều kiện để Printed $\rightarrow$ Signed và để cấp access code.
...
\attr{ReservationID} & \sqltype{BIGINT} & Đặt chỗ nguồn (sinh đúng 1 HĐ gốc) & FK $\rightarrow$ RESERVATIONS, UNIQUE (1:1) \\ \hline
...
\attr{Status} & \sqltype{ENUM('DRAFT','PRINTED','SIGNED','ACTIVE','CLOSED','SUPERSEDED')} & Trạng thái (6 giá trị) & NOT NULL \\ \hline
\end{xltabular}

\subsubsection{CONTRACT\_ADDENDUMS (PHỤ LỤC HỢP ĐỒNG -- CT-…-A1)}
\textbf{Mô tả:} Phụ lục hợp đồng -- bản ghi riêng (V3 2026-09-18), thuộc về một hợp đồng gốc, hiện chỉ sinh từ nghiệp vụ \textbf{gia hạn}: auto-draft ngay khi phí gia hạn được xác nhận, hạn ký tại quầy 7 ngày. Quá hạn \textit{không tự đóng}: ngày kết thúc đã dịch từ lúc trả phí nên phụ lục chỉ là hồ sơ giấy; hồ sơ không ký được staff đóng ở một trong hai terminal: \textbf{Expired} (khách không đến ký sau khi hết nhắc) hoặc \textbf{Voided} (staff huỷ chủ động). Quan hệ 1:N với \entity{extensions} (huỷ bản soạn nhầm rồi soạn lại = bản ghi mới cùng lần gia hạn; để NULL cho các phụ lục loại khác trong tương lai).

\begin{xltabular}{\textwidth}{|p{4.4cm}|p{3.6cm}|X|p{3.4cm}|}
\caption{Thuộc tính thực thể CONTRACT\_ADDENDUMS} \\
\hline
\textbf{Thuộc tính} & \textbf{Kiểu dữ liệu} & \textbf{Mô tả} & \textbf{Ràng buộc} \\ \hline
\endfirsthead
\hline
\textbf{Thuộc tính} & \textbf{Kiểu dữ liệu} & \textbf{Mô tả} & \textbf{Ràng buộc} \\ \hline
\endhead
\attr{AddendumID} & \sqltype{BIGINT} & Mã phụ lục & PK, AUTO\_INCREMENT \\ \hline
\attr{ContractID} & \sqltype{BIGINT} & Hợp đồng gốc & FK $\rightarrow$ CONTRACTS \\ \hline
\attr{ExtensionID} & \sqltype{BIGINT} & Lần gia hạn (nếu là phụ lục gia hạn) & FK $\rightarrow$ EXTENSIONS, NULL (1:N) \\ \hline
\attr{ContentSnapshot} & \sqltype{TEXT} & Nội dung snapshot (ngày mới\ldots) & NOT NULL \\ \hline
\attr{SignedPhotoUrl} & \sqltype{VARCHAR(255)} & Ảnh bản ký phụ lục & NULL \\ \hline
\attr{SignatureDueDate} & \sqltype{DATE} & Hạn ký 7 ngày & NOT NULL \\ \hline
\attr{Status} & \sqltype{ENUM('AWAITING\_SIGNATURE','SIGNED','EXPIRED','VOIDED')} & Trạng thái & NOT NULL \\ \hline
\end{xltabular}

\subsubsection{PAYMENTS (GIAO DỊCH THANH TOÁN)}
\textbf{Mô tả:} Mọi dòng tiền đi qua bảng này -- dùng 3 cột tham chiếu không bắt buộc (reservation/extension/settlement) thay vì đa hình chuỗi; tiền cọc lẫn tiền thuê 100\% đều tham chiếu \attr{ReservationID} (V3: gộp rental). Mỗi giao dịch thuộc đúng \textbf{một} mục đích. Cổng thanh toán mô phỏng 3 phương thức; trạng thái chạy theo state chart Payment (Pending $\rightarrow$ Processing $\rightarrow$ Succeeded/Failed/Expired).
...
\attr{ReservationID} & \sqltype{BIGINT} & Cọc đặt chỗ + tiền thuê check-in & FK $\rightarrow$ RESERVATIONS, NULL \\ \hline
...
\multicolumn{4}{|l|}{\sqltype{CHECK}: đúng một trong ba cột tham chiếu nghiệp vụ NOT NULL} \\ \hline
\end{xltabular}

\subsubsection{SETTLEMENTS (THANH LÝ TRẢ KHO)}
\textbf{Mô tả:} Bản thanh lý mỗi lần trả kho: kiểm tra hạng mục, tính phí hư hại (chỉ thu khi có lý do bắt buộc), đối chiếu số hợp đồng và hoàn phần cọc còn lại. Quan hệ 1:0..1 với reservation (mỗi phiên đặt chỗ -- thuê thanh lý một lần).
...
\attr{ReservationID} & \sqltype{BIGINT} & Phiên đặt chỗ -- thuê & FK $\rightarrow$ RESERVATIONS, UNIQUE (1:1) \\ \hline
...
\attr{ReservationID} & \sqltype{BIGINT} & Phiên đặt chỗ -- thuê (ngữ cảnh) & FK $\rightarrow$ RESERVATIONS, NULL \\ \hline
...
\textbf{Mô tả:} Staff được phép escalate ticket cho manager nhưng \textbf{bắt buộc kèm ghi chú}. Manager quyết định: mức severe (unit $\rightarrow$ Maintenance + tái định vị khách, phân công lại NV xử lý) hoặc trả về staff kèm hướng dẫn. Quyết định ghi vào Activity Log.
...
\attr{RefCode} & \sqltype{VARCHAR(20)} & Mã hồ sơ liên quan (BK-/SR-/CT-) & NULL \\ \hline
...
Tổng cộng \textbf{31 mối quan hệ} (bao gồm 1 quan hệ tự tham chiếu):
...
\entity{units} & được giữ & \entity{reservations} & 1:N & Unit bị khoá trong thời gian giữ chỗ (và được thuê sau check-in) \\ \hline
\entity{reservations} & gia hạn & \entity{extensions} & 1:N & Chuỗi kiểm toán mỗi lần kéo dài \\ \hline
\entity{reservations} & xin trả & \entity{checkout\_requests} & 1:N & Yêu cầu trả kho của khách \\ \hline
\entity{reservations} & sinh & \entity{contracts} & 1:1 & Auto-draft đúng một hợp đồng gốc mỗi đặt chỗ \\ \hline
\entity{contracts} & có & \entity{contract\_addendums} & 1:N & Hợp đồng gốc kèm các phụ lục \\ \hline
\entity{extensions} & kèm theo & \entity{contract\_addendums} & 1:N & Phụ lục gia hạn (huỷ soạn nhầm rồi soạn lại = bản ghi mới) \\ \hline
...
\entity{reservations} & thanh toán & \entity{payments} & 1:N & Cọc 10\% lúc đặt + tiền thuê 100\% tại check-in \\ \hline
...
\entity{reservations} & thanh lý & \entity{settlements} & 1:0..1 & Mỗi phiên đặt chỗ -- thuê thanh lý một lần \\ \hline
...
\entity{reservations} & thuộc phiên thuê & \entity{support\_tickets} & 1:N & Ngữ cảnh phiên đặt chỗ -- thuê của ticket \\ \hline
...
Sơ đồ đầy đủ 22 thực thể, 31 mối quan hệ (trang ngang):
...
Tài liệu này gồm \textbf{7 sơ đồ trạng thái} cho 7 thực thể có vòng đời nghiệp vụ quan trọng nhất: Unit, Reservation (gộp vòng đời Rental), Contract, Contract Addendum, Support Ticket, Task, Payment. Trạng thái lưu trong các cột \sqltype{ENUM} đã liệt kê ở Section 2 -- mỗi giá trị enum khớp đúng một trạng thái trên sơ đồ.
...
Reserved & check-in hoàn tất [hợp đồng đã ký] & Rented & Cấp access code; hợp đồng Active \\ \hline
...
\subsection{State chart: Reservation (đặt chỗ -- thuê -- BK-)}

Theo quyết định V3 (2026-09-18), vòng đời Rental cũ được \textbf{gộp vào reservation}: một bản ghi đi trọn đặt chỗ $\rightarrow$ thuê $\rightarrow$ trả, vì cọc duy trì phiên (không tồn tại ``booking không có phiên thuê''). Khách duyệt unit trên Browse rồi xác nhận Booking Summary: reservation sinh ra ở PendingPayment và chỉ thực sự giữ unit khi cổng thanh toán xác nhận cọc 10\%. Sau khi Reserved, có ba lối ra: check-in (trả 100\% + ký hợp đồng $\rightarrow$ CheckedIn, bản ghi nắm access code), quá hạn check-in (Expired, unit trả về Available), hoặc huỷ theo policy (Cancelled -- dự phòng, v1 không kích hoạt). Từ CheckedIn, gia hạn chỉ dịch \attr{EndDate} (kèm extension + phụ lục chờ ký); kết thúc phiên phải qua CheckoutRequested + settlement. Guard chống xung đột lịch: ngày trả mới nhất luôn thắng (``Latest new checkout'').
...
Reserved & trả 100\% rent + ký hợp đồng & CheckedIn & Cấp access code; hợp đồng Active (gộp vòng đời Rental) \\ \hline
CheckedIn & gia hạn [phí đã trả: \attr{EndDate} dịch ngay] & CheckedIn & Sinh extension + phụ lục chờ ký \\ \hline
CheckedIn & khách gửi checkout request & CheckoutRequested & Sinh task checkout TL- \\ \hline
...
Reserved & huỷ đặt & Cancelled & Hoàn cọc theo policy (dự phòng -- v1 không kích hoạt) \\ \hline
Expired / Closed & -- & $[\circ]$ & Kết thúc phiên đặt chỗ -- thuê \\ \hline
\end{xltabular}
...
\subsection{State chart: Contract Addendum (phụ lục -- CT-…-A1)}

V3 (2026-09-18) tách vòng đời phụ lục khỏi hợp đồng gốc -- bản ghi riêng ở bảng \entity{contract\_addendums}. Phụ lục hiện chỉ sinh từ nghiệp vụ \textbf{gia hạn}: auto-draft ngay khi phí gia hạn được xác nhận, chờ ký tại quầy với hạn 7 ngày (bell reminder trước hạn). Quá hạn \textbf{không tự đóng} -- \attr{EndDate} của reservation đã dịch ngay từ lúc trả phí nên phụ lục chỉ là hồ sơ giấy, không giữ unit; hệ thống chỉ banner amber + nhắc việc lặp lại, khách đến trễ vẫn ký được bình thường. Hồ sơ sẽ không treo vĩnh viễn: staff chủ động đóng ở một trong hai terminal -- \textbf{Expired} (khách không đến ký sau khi hết nhắc) hoặc \textbf{Voided} (soạn nhầm phải huỷ soạn lại, hoặc khách trả kho sớm khiến phụ lục vô nghĩa).

\begin{xltabular}{\textwidth}{|p{2.6cm}|p{4.4cm}|p{2.6cm}|X|}
\caption{Bảng chuyển dịch trạng thái của Contract Addendum} \\
\hline
\textbf{Từ trạng thái} & \textbf{Sự kiện [guard]} & \textbf{Đến} & \textbf{Tác dụng phụ của hệ thống} \\ \hline
\endfirsthead
\hline
\textbf{Từ trạng thái} & \textbf{Sự kiện [guard]} & \textbf{Đến} & \textbf{Tác dụng phụ của hệ thống} \\ \hline
\endhead
$[\bullet]$ & phí gia hạn thành công [auto-draft] & AwaitingSignature & Hạn ký 7 ngày (\attr{SignatureDueDate}); bell reminder \\ \hline
AwaitingSignature & ký tại quầy + staff chụp ảnh attach & Signed & Lưu \attr{SignedPhotoUrl} \\ \hline
AwaitingSignature & quá hạn & AwaitingSignature & Banner amber + nhắc việc (tự vòng, không tự đóng) \\ \hline
AwaitingSignature & không đến ký [staff đóng hồ sơ, sau khi hết nhắc] & Expired & Ghi Activity Log; khách đến sau đó phải soạn phụ lục mới \\ \hline
AwaitingSignature & staff huỷ [soạn nhầm / khách trả kho sớm] & Voided & Soạn lại = bản ghi mới cùng extension (1:N) \\ \hline
Signed & -- & $[\circ]$ & Phụ lục hoàn tất hồ sơ giấy \\ \hline
Expired / Voided & -- & $[\circ]$ & Đóng hồ sơ phụ lục (lý do ghi Activity Log) \\ \hline
\end{xltabular}

\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{state-addendum.png}
\caption{State chart Contract Addendum}
\label{fig:state-addendum}
\end{figure}

\subsection{State chart: Contract (hợp đồng gốc -- CT-)}

Hợp đồng gốc: auto-draft ngay khi đặt cọc thành công (khoá phiên bản policy), staff in tại quầy, khách ký + staff chụp ảnh attach, check-in hoàn tất thì hợp đồng Active; settlement đối chiếu số hợp đồng khi đóng. \textbf{Hợp đồng không ai sửa được} -- muốn sửa phải draft lại; vì thế gia hạn đi theo \textbf{phụ lục (addendum)}: bản ghi riêng ở bảng \entity{contract\_addendums}, vòng đời ở state chart Addendum (mục trên), ký xong hợp đồng gốc bị Superseded về ngày kết thúc.

Ba điểm bổ sung: (1) \textit{in lại} không đổi trạng thái -- bản in là dữ kiện một khi đã đúng thì đúng mãi, mỗi lần in chỉ ghi Activity Log; (2) nếu booking bị huỷ hoặc quá hạn check-in trước khi ký, hợp đồng gốc (Draft/Printed) bị Closed theo policy hoàn cọc, không treo lơ lửng; (3) Superseded là điểm kết thúc -- bản gốc ngừng hiệu lực về ngày kết thúc, hồ sơ vẫn giữ để đối chiếu.
...
Printed & in lại [giấy mất / khách chưa ký] & Printed & Không đổi trạng thái; mỗi lần in ghi Activity Log \\ \hline
Printed & khách ký + staff chụp ảnh attach & Signed & Lưu \attr{SignedPhotoUrl} \\ \hline
Signed & check-in hoàn tất (reservation CHECKED\_IN) & Active & Hợp đồng gốc có hiệu lực \\ \hline
...
Draft & booking huỷ / quá hạn check-in [hoàn cọc theo policy] & Closed & Đóng hợp đồng gốc chưa hiệu lực \\ \hline
Printed & booking huỷ / quá hạn check-in [hoàn cọc theo policy] & Closed & Đóng; không cấp access code \\ \hline
Closed / Superseded & -- & $[\circ]$ & Kết thúc vòng đời hợp đồng \\ \hline
\end{xltabular}
...
Ticket vào Open khi khách gửi support request, được định tuyến tới staff trực ca theo unit + shift. Staff xử lý xong thì Resolved (kèm note); nếu vượt thẩm quyền thì Escalated -- \textbf{bắt buộc có note}. Manager luôn đưa ticket về lại nhân viên: quyết định severe (unit $\rightarrow$ Maintenance + tái định vị khách + phân công xử lý) hay trả về kèm guidance, ticket đều quay lại InProgress và chỉ Resolved khi staff hoàn tất phần việc của mình. Ticket severe không bao giờ bị bỏ rơi: quyết định ghi Activity Log, khách nhận notification từng bước. Danh sách ticket phía khách hiển thị thêm nhãn xám ``Closed'' cho ticket Resolved cũ -- đó là trạng thái hiển thị (archive), không phải giá trị enum riêng.
...
Escalated & manager quyết severe + phân công NV & InProgress & Unit $\rightarrow$ Maintenance; tái định vị khách; sinh task xử lý; Activity Log \\ \hline
...
Cả 4 touchpoint tiền (cọc 10\%, thuê 100\%, phí gia hạn, phí phát sinh vượt cọc khi thanh lý) dùng chung một máy trạng thái của Payment Modal. Bấm Pay khóa nút back, spinner 1.5--2s; gateway mô phỏng trả Succeeded hoặc Failed (``No money was taken''); QR hết hạn sau khoảng 5 phút thì về Expired và khách chọn lại method. Chỉ Succeeded mới đổi trạng thái nghiệp vụ (reservation/extension/settlement) và bắn toast + bell.
...
\textbf{Từ trạng thái} & \textbf{Sự kiện [guard]} & \textbf{Đến} & \textbf{Tác dụng phụ của hệ thống} \\ \hline
\endhead
$[\bullet]$ & mở Payment Modal [deposit / rent / extension fee / extra fee] & Pending & Chọn method (Card/MoMo/VNPay) \\ \hline
...
%% StorageHub - ERD (crow's foot) - 22 thuc the, 8 nhom - V3 2026-09-18
%% Thay doi V3: gop RENTALS vao RESERVATIONS (1 bang tron vong doi);
%% tach CONTRACT_ADDENDUMS khoi CONTRACTS (bo self-FK); RESERVATIONS 1:1 CONTRACTS.
erDiagram
    ...
    %% ── Nhom 3: Dat & thue (V3: gop Rental) ──
    USERS ||--o{ RESERVATIONS : "dat"
    UNITS ||--o{ RESERVATIONS : "duoc giu"
    RESERVATIONS ||--o{ EXTENSIONS : "gia han"
    RESERVATIONS ||--o{ CHECKOUT_REQUESTS : "xin tra"

    %% ── Nhom 4: Hop dong & phu luc (V3: tach Addendum) ──
    RESERVATIONS ||--|| CONTRACTS : "sinh"
    RENTAL_POLICIES ||--o{ CONTRACTS : "khoa phien ban"
    CONTRACTS ||--o{ CONTRACT_ADDENDUMS : "co"
    EXTENSIONS ||--o{ CONTRACT_ADDENDUMS : "kem theo"

    %% ── Nhom 5: Tien ──
    USERS ||--o{ PAYMENTS : "thanh toan"
    RESERVATIONS ||--o{ PAYMENTS : "coc 10% + thue 100%"
    EXTENSIONS ||--o| PAYMENTS : "phi gia han"
    SETTLEMENTS ||--o{ PAYMENTS : "damage / extra"

    %% ── Nhom 6: Tra kho & thanh ly ──
    RESERVATIONS ||--o| SETTLEMENTS : "thanh ly"
    ...
    RESERVATIONS ||--o{ SUPPORT_TICKETS : "thuoc phien thue"
...
\begin{Verbatim}[fontsize=\small]
%% StorageHub - State chart: vong doi Reservation (BK-) - V3 2026-09-18
%% Gop vong doi Rental cu: coc duy tri phien, CheckedIn = dang thue
stateDiagram-v2
    direction LR
    ...
    Reserved --> CheckedIn : tra 100% rent + ky hop dong (cap access code)
    Reserved --> Expired : qua han check-in [unit tra ve Available]
    Reserved --> Cancelled : huy dat [hoan deposit theo policy - du phong, v1 khong kich hoat]

    CheckedIn --> CheckedIn : gia han [fee da tra: EndDate dich ngay]
                                  (sinh extension + addendum cho ky)
    CheckedIn --> CheckoutRequested : KH gui checkout request
    CheckoutRequested --> Closed : settlement hoan tat [refund = deposit - damage fee]

    Closed --> [*]
    Expired --> [*]

    note right of CheckedIn
        CheckedIn = trang thai "dang thue"
        (Rental cu gop vao Reservation - V3):
        giu coc, AccessCode, EndDate
        dich ngay khi gia han.
...
\subsection{Mermaid \#4 -- State chart: Contract Addendum}

\begin{mermaidbox}{Mermaid: State chart Contract Addendum}
\begin{Verbatim}[fontsize=\small]
%% StorageHub - State chart: vong doi Contract Addendum (CT-...-A1) - V3 2026-09-18
%% Tach khoi vong doi Contract goc; sinh tu Extension, khong hold unit
stateDiagram-v2
    direction LR
    [*] --> AwaitingSignature : phi gia han thanh cong [auto-draft, han ky 7 ngay]

    AwaitingSignature --> Signed : ky tai quay + staff chup anh attach
    AwaitingSignature --> AwaitingSignature : qua han [banner amber + nhac viec]
    AwaitingSignature --> Expired : khong den ky [staff dong ho so, sau khi het nhac]
    AwaitingSignature --> Voided : staff huy [soan nham / khach tra kho som]

    Signed --> [*] : phu luc hoan tat ho so giay
    Expired --> [*] : dong ho so khong ky
    Voided --> [*] : dong ho so (soan lai = ban ghi moi)

    note right of AwaitingSignature
        Qua han KHONG tu dong: EndDate da dich
        tu luc tra phi, phu luc chi la ho so giay -
        khong hold unit; khach den tre van ky duoc.
        Staff dong tay: Expired (khong den ky)
        hoac Voided (soan nham / tra kho som).
        Bell reminder truoc han.
    end note
\end{Verbatim}
\end{mermaidbox}
...
\begin{Verbatim}[fontsize=\small]
%% StorageHub - State chart: vong doi Contract goc (CT-) - V3 2026-09-18
%% Phu luc tach sang bang rieng - xem state chart Addendum
stateDiagram-v2
    direction LR
    [*] --> Draft : auto-draft [ngay khi deposit thanh cong, khoa policy version]

    Draft --> Printed : staff in tai quay
    Printed --> Printed : in lai [giay mat / KH chua ky]
    Printed --> Signed : KH ky + staff chup anh attach
    Signed --> Active : check-in hoan tat (reservation CHECKED_IN)
    Active --> Closed : settlement tham chieu so hop dong
    Active --> Superseded : addendum thay ngay ket thuc

    Draft --> Closed : booking huy / qua han check-in [hoan coc theo policy]
    Printed --> Closed : booking huy / qua han check-in
    ...
    note right of Printed
        ...
        chinh = draft lai; thay doi
        ngay ket thuc di qua phu luc
        (state chart Addendum rieng).
    end note
\end{Verbatim}
\end{mermaidbox}
...
stateDiagram-v2
    ...
    Escalated --> InProgress : manager quyet severe
...
\begin{Verbatim}[fontsize=\small]
%% StorageHub - State chart: vong doi Payment (mock gateway, 4 touchpoint)
stateDiagram-v2
    [*] --> Pending : mo Payment Modal [deposit / rent / extension fee / extra fee]
