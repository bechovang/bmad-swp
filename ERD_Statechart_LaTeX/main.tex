...
\textbf{Mô tả:} Phụ lục hợp đồng -- bản ghi riêng (V3 2026-09-18), thuộc về một hợp đồng gốc, hiện chỉ sinh từ nghiệp vụ \textbf{gia hạn}: auto-draft ngay khi phí gia hạn được xác nhận, hạn ký tại quầy 7 ngày. Quá hạn ký \textit{không có terminal}: ngày kết thúc đã dịch từ lúc trả phí nên phụ lục chỉ là hồ sơ giấy. Quan hệ tùy chọn 1:0..1 với \entity{extensions} (một lần gia hạn có tối đa một phụ lục; để NULL cho các phụ lục loại khác trong tương lai).
...
\attr{ExtensionID} & \sqltype{BIGINT} & Lần gia hạn (nếu là phụ lục gia hạn) & FK $\rightarrow$ EXTENSIONS, UNIQUE (0..1), NULL \\ \hline
...
\attr{Status} & \sqltype{ENUM('AWAITING\_SIGNATURE','SIGNED')} & Trạng thái & NOT NULL \\ \hline
\end{xltabular}
...
\entity{extensions} & kèm theo & \entity{contract\_addendums} & 1:0..1 & Phụ lục gia hạn (tùy chọn) \\ \hline
...
V3 (2026-09-18) tách vòng đời phụ lục khỏi hợp đồng gốc -- bản ghi riêng ở bảng \entity{contract\_addendums}. Phụ lục hiện chỉ sinh từ nghiệp vụ \textbf{gia hạn}: auto-draft ngay khi phí gia hạn được xác nhận, chờ ký tại quầy với hạn 7 ngày (bell reminder trước hạn). Điểm thiết kế quan trọng: \textbf{quá hạn ký không có terminal} -- \attr{EndDate} của reservation đã dịch ngay từ lúc trả phí nên phụ lục chỉ là hồ sơ giấy, không giữ unit; hệ thống chỉ banner amber + nhắc việc lặp lại.
...
AwaitingSignature & quá hạn & AwaitingSignature & Banner amber + nhắc việc (tự vòng, không terminal) \\ \hline
...
stateDiagram-v2
    ...
    note right of AwaitingSignature
        Qua han khong co terminal:
        EndDate da dich tu luc tra phi,
        phu luc chi la ho so giay -
        khong hold unit.
