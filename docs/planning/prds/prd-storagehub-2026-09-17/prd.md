...
- **Extension** — Yêu cầu gia hạn thuộc một Rental. Ngày checkout mới có hiệu lực **ngay sau khi thanh toán phí gia hạn**; sinh **Addendum** (nếu phụ lục bị Voided, soạn lại = bản ghi mới cùng Extension). Trạng thái: PENDING_PAYMENT → APPLIED (đóng modal chưa trả = không ghi nhận; không có CANCELLED trong v1). Deposit giữ nguyên theo booking gốc.
